...
let neighbours current = 
    ...
    let point = __YOUR_IMPLEMENTION_HERE__
...
let newState current getElevation point =
    { current with
        ...
        Elevation = __YOUR_IMPLEMENTION_HERE__ }
...
let findHigherPoint current getElevation =
    ...
    if __YOUR_IMPLEMENTION_HERE__ then
        best
    else
        { current with Step = __YOUR_IMPLEMENTION_HERE__ }
    ...
    let rec climbUntilDone state = 
        seq { yield state
              ...
              if not (__YOUR_IMPLEMENTION_HERE__) then
                  ...
                  yield! __YOUR_IMPLEMENTION_HERE__ }
